...
public interface ProductRepository extends JpaRepository<Product, Long> {
	...
	@Query("select p from Product p join p.reviews r where (:code is null or upper(p.code) like upper(concat('%', :code, '%') )) and (:name is null or upper(p.name) like upper(concat('%', :name, '%')))")
